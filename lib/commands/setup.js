...
function setup() {
  ...
  if (installed.length) {
    const bash = installed.find(t => t.type === 'bash');
    const ps = installed.find(t => t.type === 'powershell');
    console.log('Done. Restart your shell to activate the wrapper.');
    if (bash) console.log(`  bash/zsh: source ${bash.path}`);
    if (ps) console.log(`  powershell: . $PROFILE`);
  } else {
    console.log('Done.');
  }
}
